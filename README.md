...
simply run `run.sh` and navigate to localhost:8080
...
To launch:  
Run `greycat serve` in one terminal to run the backend.  
Run `pnpm gen` to generate typing for your project.  
Run `pnpm dev` to launch the front in dev mode.

Note: it is necessary to run again `pmpm gen` each time you change the types and/or exposed functions in your backend if you want the changes to be visible in your frontend code.
